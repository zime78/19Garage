...
import com.zime.garage.ui.data.type.VehicleFormatType
...
class VehicleFormatModel: BaseModel() {
    ...
    /**
     * 기본 차량 형식 파일 설정
     * 파일이 존재하지 않을 경우 기본 차량 형식 항목들로 새 파일을 생성합니다.
        ...
        file?.let {
            ...
            try {
                if (file.exists()) {
                    ...
                    val currentTypes = loadVehicleFormatFile().map { it.type }
                    val missingTypes = defaultTypes.filter { !currentTypes.contains(it) }
                    
                    if (missingTypes.isNotEmpty()) {
                        val success = addMultipleVehicleFormatTypes(missingTypes)
                        if (DEBUG_LOG && success) {
                            println("누락된 기본 차량 형식 항목들이 추가되었습니다: ${missingTypes.joinToString(", ")}")
                        }
                    }
                    
                    if (DEBUG_LOG) {
                        LocalFileManager.logFileLoadAll(file)
                        println("차량 형식 파일이 이미 존재합니다.")
                    }
                } else {
    ...
    /**
     * 차량 형식 항목을 JSON 파일에 저장
     * @param types : 저장할 차량 형식 항목 문자열 리스트
     * @return 저장 성공 여부
     */
    fun saveVehicleFormatFile(types: List<String>): Boolean {
        val file = openFile(FileType.VEHICLE_FORMAT)
        return file?.let {
            try {
                // 중복 제거 및 정렬
                ...
                if (DEBUG_LOG) {
                    println("차량 형식 파일 저장 완료: ${uniqueTypes.size}개 항목")
                    LocalFileManager.logFileLoadAll(file)
                }
                true
            } catch (e: Exception) {
                println("saveVehicleFormatFile 저장 오류: $e")
                false
            } finally {
                runBlocking {
                    closeFile(file)
    ...
    /**
     * 차량 형식 항목 추가
     * @param newType : 추가할 차량 형식 항목
     * @return 추가 성공 여부
     */
    fun addVehicleFormatType(newType: String): Boolean {
        if (newType.isBlank()) {
            println("빈 차량 형식 항목은 추가할 수 없습니다.")
            return false
        }
        
        val currentTypes = loadVehicleFormatFile().map { it.type }.toMutableList()
        
        // 중복 확인
        if (currentTypes.contains(newType)) {
            println("이미 존재하는 차량 형식 항목입니다: $newType")
            return false
        }
        
        currentTypes.add(newType)
        return saveVehicleFormatFile(currentTypes)
    }

    /**
     * 차량 형식 항목 수정
     * @param oldType : 기존 차량 형식 항목
     * @param newType : 새로운 차량 형식 항목
     * @return 수정 성공 여부
     */
    fun updateVehicleFormatType(oldType: String, newType: String): Boolean {
        if (newType.isBlank()) {
            println("빈 차량 형식 항목으로 수정할 수 없습니다.")
            return false
        }
        ...
        if (index == -1) {
            println("수정할 차량 형식 항목을 찾을 수 없습니다: $oldType")
            return false
        }
    ...
    /**
     * 차량 형식 항목 삭제
     * @param typeToDelete : 삭제할 차량 형식 항목
     * @return 삭제 성공 여부
     */
    fun deleteVehicleFormatType(typeToDelete: String): Boolean {
        val currentTypes = loadVehicleFormatFile().map { it.type }.toMutableList()
        
        // 항목 존재 확인
        if (!currentTypes.contains(typeToDelete)) {
            println("삭제할 차량 형식 항목을 찾을 수 없습니다: $typeToDelete")
            return false
        }
    ...
    /**
     * 여러 차량 형식 항목 일괄 추가
     * @param newTypes : 추가할 차량 형식 항목 리스트
     * @return 추가 성공 여부
     */
    fun addMultipleVehicleFormatTypes(newTypes: List<String>): Boolean {
        val validTypes = newTypes.filter { it.isNotBlank() }
        if (validTypes.isEmpty()) {
            println("추가할 유효한 차량 형식 항목이 없습니다.")
            return false
        }
        ...
        val success = saveVehicleFormatFile(currentTypes)
        
        if (success) {
            println("${typesToAdd.size}개의 차량 형식 항목이 추가되었습니다: ${typesToAdd.joinToString(", ")}")
        }
