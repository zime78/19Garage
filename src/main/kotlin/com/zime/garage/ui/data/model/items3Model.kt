...
import com.zime.garage.ui.data.type.ClassificationType
...
class Items3Model(): BaseModel() {
    ...
    fun setDefaultItemsFile() {
        val file = openFile(FileType.ITEMS3)
        file?.let {
            val defaultTypes = listOf(
                "교체","엔진오일","에어컨","수리","오디오헤드","센서류","스피커","기타","엔진","설치","TCU","리모컨","스티어링","미션오일"
                ,"크리닝","배터리","부싱","점검","장착","연료첨가제","판넬","냉각수","보수","서스","램프류","서스팬션","전구류","헤드라이트"
                ,"팔걸이","터보","브레이크","안개등","출력상승","하체","공임","트렁크","외장","A/S","계기판","조향","스위치","연료","샘"
                ,"머플러","헤드","필터","휴즈","스프링"
            )
            try {
                if (file.exists()) {
    ...
    fun loadClassificationFile(): List<ClassificationType> {
        val file = openFile(FileType.ITEMS3) // 적절한 파일 열기 함수 호출
        return file?.let {
            try {
                if (!file.exists()) {
